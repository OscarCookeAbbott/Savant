...
`};r.backslashEscapesHTMLTags&&(e=e.replace(/\\<(\/?[^>]+?)>/g,function(l,C){return"&lt;"+C+"&gt;"}));for(var f=0;f<i.length;++f)for(var s,p=new RegExp("^ {0,3}(<"+i[f]+"\\b[^>]*>)","im"),m="<"+i[f]+"\\b[^>]*>",y="</"+i[f]+">";(s=n.helper.regexIndexOf(e,p))!==-1;){var A=n.helper.splitAtIndex(e,s),T=n.helper.replaceRecursiveRegExp(A[1],o,m,y,"im");if(T===A[1])break;e=A[0].concat(T)}return e=e.replace(/(\n {0,3}(<(hr)\b([^<>])*?\/?>)[ \t]*(?=\n{2,}))/g,n.subParser("hashElement")(e,r,a)),e=n.helper.replaceRecursiveRegExp(e,function(l){return`
...
`)):e=e.replace(/  +\n/g,`<br />
`),e=a.converter._dispatch("spanGamut.after",e,r,a),e}),n.subParser("strikethrough",function(e,r,a){function i(o){return r.simplifiedAutoLink&&(o=n.subParser("simplifiedAutoLinks")(o,r,a)),"<del>"+o+"</del>"}return r.strikethrough&&(e=a.converter._dispatch("strikethrough.before",e,r,a),e=e.replace(/(?:~){2}([\s\S]+?)(?:~){2}/g,function(o,f){return i(f)}),e=a.converter._dispatch("strikethrough.after",e,r,a)),e}),n.subParser("stripLinkDefinitions",function(e,r,a){var i=/^ {0,3}\[([^\]]+)]:[ \t]*\n?[ \t]*<?([^>\s]+)>?(?: =([*\d]+[A-Za-z%]{0,4})x([*\d]+[A-Za-z%]{0,4}))?[ \t]*\n?[ \t]*(?:(\n*)["|'(](.+?)["|')][ \t]*)?(?:\n+|(?=¨0))/gm,o=/^ {0,3}\[([^\]]+)]:[ \t]*\n?[ \t]*<?(data:.+?\/.+?;base64,[A-Za-z0-9+/=\n]+?)>?(?: =([*\d]+[A-Za-z%]{0,4})x([*\d]+[A-Za-z%]{0,4}))?[ \t]*\n?[ \t]*(?:(\n*)["|'(](.+?)["|')][ \t]*)?(?:\n\n|(?=¨0)|(?=\n\[))/gm;e+="¨0";var f=function(s,p,m,y,A,T,l){return p=p.toLowerCase(),e.toLowerCase().split(p).length-1<2?s:(m.match(/^data:.+?\/.+?;base64,/)?a.gUrls[p]=m.replace(/\s/g,""):a.gUrls[p]=n.subParser("encodeAmpsAndAngles")(m,r,a),T?T+l:(l&&(a.gTitles[p]=l.replace(/"|'/g,"&quot;")),r.parseImgDimensions&&y&&A&&(a.gDimensions[p]={width:y,height:A}),""))};return e=e.replace(o,f),e=e.replace(i,f),e=e.replace(/¨0/,""),e}),n.subParser("tables",function(e,r,a){if(!r.tables)return e;var i=/^ {0,3}\|?.+\|.+\n {0,3}\|?[ \t]*:?[ \t]*(?:[-=]){2,}[ \t]*:?[ \t]*\|[ \t]*:?[ \t]*(?:[-=]){2,}[\s\S]+?(?:\n\n|¨0)/gm,o=/^ {0,3}\|.+\|[ \t]*\n {0,3}\|[ \t]*:?[ \t]*(?:[-=]){2,}[ \t]*:?[ \t]*\|[ \t]*\n( {0,3}\|.+\|[ \t]*\n)*(?:\n|¨0)/gm;function f(A){return/^:[ \t]*--*$/.test(A)?' style="text-align:left;"':/^--*[ \t]*:[ \t]*$/.test(A)?' style="text-align:right;"':/^:[ \t]*--*[ \t]*:$/.test(A)?' style="text-align:center;"':""}function s(A,T){var l="";return A=A.trim(),(r.tablesHeaderId||r.tableHeaderId)&&(l=' id="'+A.replace(/ /g,"_").toLowerCase()+'"'),A=n.subParser("spanGamut")(A,r,a),"<th"+l+T+">"+A+`</th>
`}function p(A,T){var l=n.subParser("spanGamut")(A,r,a);return"<td"+T+">"+l+`</td>
`}function m(A,T){for(var l=`<table>
<thead>
<tr>
`,C=A.length,L=0;L<C;++L)l+=A[L];for(l+=`</tr>
</thead>
<tbody>
`,L=0;L<T.length;++L){l+=`<tr>
`;for(var R=0;R<C;++R)l+=T[L][R];l+=`</tr>
`}return l+=`</tbody>
</table>
`,l}function y(A){var T,l=A.split(`
`);for(T=0;T<l.length;++T)/^ {0,3}\|/.test(l[T])&&(l[T]=l[T].replace(/^ {0,3}\|/,"")),/\|[ \t]*$/.test(l[T])&&(l[T]=l[T].replace(/\|[ \t]*$/,"")),l[T]=n.subParser("codeSpans")(l[T],r,a);var C=l[0].split("|").map(function(v){return v.trim()}),L=l[1].split("|").map(function(v){return v.trim()}),R=[],U=[],$=[],X=[];for(l.shift(),l.shift(),T=0;T<l.length;++T)l[T].trim()!==""&&R.push(l[T].split("|").map(function(v){return v.trim()}));if(C.length<L.length)return A;for(T=0;T<L.length;++T)$.push(f(L[T]));for(T=0;T<C.length;++T)n.helper.isUndefined($[T])&&($[T]=""),U.push(s(C[T],$[T]));for(T=0;T<R.length;++T){for(var Q=[],K=0;K<U.length;++K)n.helper.isUndefined(R[T][K]),Q.push(p(R[T][K],$[K]));X.push(Q)}return m(U,X)}return e=a.converter._dispatch("tables.before",e,r,a),e=e.replace(/\\(\|)/g,n.helper.escapeCharactersCallback),e=e.replace(i,y),e=e.replace(o,y),e=a.converter._dispatch("tables.after",e,r,a),e}),n.subParser("underline",function(e,r,a){return r.underline&&(e=a.converter._dispatch("underline.before",e,r,a),r.literalMidWordUnderscores?(e=e.replace(/\b___(\S[\s\S]*?)___\b/g,function(i,o){return"<u>"+o+"</u>"}),e=e.replace(/\b__(\S[\s\S]*?)__\b/g,function(i,o){return"<u>"+o+"</u>"})):(e=e.replace(/___(\S[\s\S]*?)___/g,function(i,o){return/\S$/.test(o)?"<u>"+o+"</u>":i}),e=e.replace(/__(\S[\s\S]*?)__/g,function(i,o){return/\S$/.test(o)?"<u>"+o+"</u>":i})),e=e.replace(/(_)/g,n.helper.escapeCharactersCallback),e=a.converter._dispatch("underline.after",e,r,a)),e}),n.subParser("unescapeSpecialChars",function(e,r,a){return e=a.converter._dispatch("unescapeSpecialChars.before",e,r,a),e=e.replace(/¨E(\d+)E/g,function(i,o){var f=parseInt(o);return String.fromCharCode(f)}),e=a.converter._dispatch("unescapeSpecialChars.after",e,r,a),e}),n.subParser("makeMarkdown.blockquote",function(e,r){var a="";if(e.hasChildNodes())for(var i=e.childNodes,o=i.length,f=0;f<o;++f){var s=n.subParser("makeMarkdown.node")(i[f],r);s!==""&&(a+=s)}return a=a.trim(),a="> "+a.split(`
`).join(`
...
`;function mt(){return G(ue({class:"language-typescript w-2xl *:scroll-m-21"},gt))}const vt=Object.freeze(Object.defineProperty({__proto__:null,default:mt},Symbol.toStringTag,{value:"Module"})),bt=`# Derive
...
`;function _t(){return G(ue({class:"language-typescript w-2xl *:scroll-m-21"},bt))}const wt=Object.freeze(Object.defineProperty({__proto__:null,default:_t},Symbol.toStringTag,{value:"Module"})),yt=`# Elements
...
`;function kt(){return G(ue({class:"language-typescript w-2xl *:scroll-m-21"},yt))}const Pt=Object.freeze(Object.defineProperty({__proto__:null,default:kt},Symbol.toStringTag,{value:"Module"})),Ct=`# Components

> Create composable functionality.
`;function St(){return G(ue({class:"language-typescript w-2xl *:scroll-m-21"},Ct))}const xt=Object.freeze(Object.defineProperty({__proto__:null,default:St},Symbol.toStringTag,{value:"Module"})),At=`# Context
...
`;function Et(){return G(ue({class:"language-typescript w-2xl *:scroll-m-21"},At))}const Tt=Object.freeze(Object.defineProperty({__proto__:null,default:Et},Symbol.toStringTag,{value:"Module"}));function jt(){const t=B(`<div>
    <p>
        👋Hello
    </p>
...
Button(
    ...
    ...children: ChildDom[]
): HTMLButtonElement`))}const Ht=Object.freeze(Object.defineProperty({__proto__:null,default:Dt},Symbol.toStringTag,{value:"Module"}));function Ut(){const t=B("variant-soft-outline"),u=B("mood-none");return G(h.h1("Checkbox"),h.blockquote("Offers clear binary choices."),h.h2("Demo"),h.div({name:"Demo",class:"card vessel flex flex-col items-center gap-4"},h.div({class:"p-8 h-48 flex items-center"},Je({class:()=>`w-48 ${t.val} ${u.val}`},"Demo Checkbox")),h.div({class:"flex flex-wrap gap-4 justify-center"},V({content:"Variant",class:"items-center"},Z({options:[{value:"variant-outline"},{value:"variant-soft"},{value:"variant-soft-outline"},{value:"variant-filled"}],value:t,class:"variant-outline w-48"})),V({content:"Mood",class:"items-center"},Z({options:[{value:"mood-none"},{value:"mood-accent"},{value:"mood-info"},{value:"mood-success"},{value:"mood-caution"},{value:"mood-critical"}],value:u,class:"variant-outline w-48"})))),()=>q({language:"ts"},`import { Checkbox } from "savant/ui"
...
Code(
    ...
    ...children: ChildDom[]
): HTMLElement`))}const qt=Object.freeze(Object.defineProperty({__proto__:null,default:Vt},Symbol.toStringTag,{value:"Module"}));function Wt(){const t=B("variant-soft-outline"),u=B("mood-none");return G(h.h1("Control Group"),h.blockquote("Brings together related interaction."),h.h2("Demo"),h.div({name:"Demo",class:"card vessel flex flex-col items-center gap-4"},h.div({class:"p-8 h-48 flex items-center"},nt({class:()=>`w-96 ${t.val} ${u.val}`},te({},"Button"),Z({value:B("Select"),options:[{value:"Select"},{value:"Also select"}]}),Ke({},"Badge"))),h.div({class:"flex flex-wrap gap-4 justify-center"},V({content:"Variant",class:"items-center"},Z({options:[{value:"variant-outline"},{value:"variant-soft"},{value:"variant-soft-outline"},{value:"variant-filled"}],value:t,class:"variant-outline w-48"})),V({content:"Mood",class:"items-center"},Z({options:[{value:"mood-none"},{value:"mood-accent"},{value:"mood-info"},{value:"mood-success"},{value:"mood-caution"},{value:"mood-critical"}],value:u,class:"variant-outline w-48"})))),()=>q({language:"ts"},`import { Switch } from "savant/ui"
...
Button(
    ...
    ...children: ChildDom[]
): HTMLElement`))}const Jt=Object.freeze(Object.defineProperty({__proto__:null,default:Kt},Symbol.toStringTag,{value:"Module"}));function Qt(){const t=B("variant-soft-outline"),u=B("mood-none"),d=B(50),n=B(!0),c=B(2);return G(h.h1("Circular Progress Bar"),h.blockquote("Displays the progress state of a lengthy process."),h.h2("Design"),h.p("Circular Progress Bars are used to compactly provide feedback to the user on the current progress state of a lengthy process."),h.p("Circular Progress Bars default to matching the size of enclosing text flows, ensuring they fit seamlessly into layouts."),h.p("Circular Progress Bars can be given children which will be displayed within the circle. Such children should be short, typically a percentage or number. Anything longer should be placed beneath the progress bar."),h.h2("Demo"),h.div({name:"Demo",class:"card vessel flex flex-col items-center gap-4"},h.div({class:"p-8 h-48 flex gap-4 items-center"},dn({progress:d,indefinite:n,thickness:c,class:()=>`p-2 ${t.val} ${u.val}`},"Loading..."),h.span({class:"flex gap-2 items-center"},dn({progress:d,indefinite:n,thickness:c,class:()=>`${t.val} ${u.val}`}),"Loading...")),h.div({class:"flex flex-wrap gap-4 justify-center"},V({content:"Variant",class:"items-center"},Z({options:[{value:"variant-outline"},{value:"variant-soft"},{value:"variant-soft-outline"},{value:"variant-filled"}],value:t,class:"variant-outline w-48"})),V({content:"Mood",class:"items-center"},Z({options:[{value:"mood-none"},{value:"mood-accent"},{value:"mood-info"},{value:"mood-success"},{value:"mood-caution"},{value:"mood-critical"}],value:u,class:"variant-outline w-48"})),V({content:"Progress",class:"items-center"},me({value:d,min:0,max:100,class:"variant-outline w-48"})),V({content:"Indefinite",class:"items-center"},Je({value:n,class:"variant-outline w-48"},"Enabled")),V({content:"Thickness",class:"items-center"},me({type:"number",value:c,class:"variant-outline w-48"})))),()=>q({language:"ts"},`import { ProgressRadial } from "savant/ui"
...
Switch(
    ...
    ...children: ChildDom[]
): HTMLElement`))}const ur=Object.freeze(Object.defineProperty({__proto__:null,default:ar},Symbol.toStringTag,{value:"Module"}));function dr(){return G(h.h1("Tooltip"),h.blockquote("Provides concise contextual extra information."),h.h2("Demo"),h.div({class:"card vessel flex flex-col items-center gap-4"},h.div({class:"p-8 flex flex-col gap-8 w-3xs h-48 justify-center"},te({class:"text-accent-500 font-bold"},"Tooltip (Hover Me)",De({direction:"top",class:"left-1/2 -translate-x-1/2"},"Demo Tooltip")))),()=>q({language:"ts"},`import { Button, Tooltip } from "savant/ui"
...
Button(
    ...
    ...children: ChildDom[]
): HTMLElement`))}const sr=Object.freeze(Object.defineProperty({__proto__:null,default:dr},Symbol.toStringTag,{value:"Module"}));function or(){const t=B(""),u=B(""),d=B(!1),n=B(!1),c=B(!1);return tt({name:"Sign In",class:"flex flex-col gap-4"},h.span({class:"text-xl font-bold"},"Sign In"),V({content:"Username"},me({type:"username",value:t,placeholder:"Enter username...",required:!0,lead:xe("person"),class:"variant-outline"})),V({content:h.span({class:"flex flex-1 justify-between items-center"},"Password",te({class:"mood-accent text-mood-500"},"Forgot?"))},me({type:()=>n.val?"text":"password",value:u,placeholder:"Enter password...",required:!0,lead:xe("key"),trail:te({onclick:()=>n.val=!n.val},xe({class:()=>`transition ${n.val?"":"text-swatch-700-foreground"}`},()=>n.val?"visibility":"visibility_off")),onValidityChanged:g=>c.val=g,class:"variant-outline"})),Je({value:d,class:"variant-outline"},h.span({class:"text-mini text-swatch-700-mood"},"Remember Password")),h.div({class:"flex gap-4 justify-end"},te({class:"hover:variant-soft"},"Cancel"),te({class:"variant-filled mood-accent"},"Sign In")))}function ir(){return G(h.h1("Sign In"),h.blockquote("Example Sign In component."),h.h2("Demo"),h.div({class:"card vessel bg-zebra zebra-opacity-20 flex flex-col items-center gap-4"},h.div({class:"p-8 card vessel bg-background flex items-center"},or())),h.h2("Code"),q({language:"typescript"},`function SignIn() {
    const username = state("")
