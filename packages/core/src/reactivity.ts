...
/** Creates a reactive effect that executes the given function whenever any of its dependencies change.
 * @note Is just syntactic sugar over `derive()`.
 */
export function effect<T = void>(
	func: () => T,
	onDispose?: () => void,
): State<T> {
	return derive(func, onDispose)
}
